...
- Remote browsing and preview of `.gitignore`-filtered text files in selected workspaces.
...
Workspace file browsing is read-only. It lists files not excluded by workspace `.gitignore` rules, groups them into navigable directories, and previews regular UTF-8 text files up to 256 KiB. The internal `.git/` directory is hidden. Binary files, symlinks, ignored files, oversized files, and invalid paths are rejected with an inline error.
...
- Workspace file browsing is limited to `.gitignore`-visible regular files inside the selected workspace and previews UTF-8 text only.
...
- `SlashCommandRouter` owns slash-command text, dispatches known commands, and reports unsupported commands to the user.
