...
import { CapabilityRegistry, parseSendImageRequest, RELAY_CAPABILITY_SEND_IMAGE } from "./capabilities.ts";
import { relayCapabilityInstructions, sendImageCapabilityInstructions } from "./control/skills.ts";
import { startControlServer, type RunningControlServer } from "./control/server.ts";
import { createMessagingAdapter } from "./messaging/provider.ts";
import { createAgentDriver } from "./agents/provider.ts";
...
export async function main(): Promise<void> {
  ...
  router = new MessageRouter({ config, store, adapter: messaging, agent, logger });
  if (config.relayControlEnabled) {
