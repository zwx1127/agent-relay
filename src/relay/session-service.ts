...
export class RelaySessionService {
  ...
  async ensureStarted(
    ...
    // Experimental Relay Work binds existing threads only through an explicit
    // /resume action. Ordinary messages start fresh instead of guessing which
    // native Codex thread the user intended.
    const resumePrevious = options.resumePrevious ?? !this.deps.experimentalRelayWorkEnabled;
