...
export class RelayController {
  ...
  constructor(private readonly deps: RelayControllerDeps) {
    ...
    this.agentEvents = new RelayAgentEventRouter({
      ...
      handlePlanDecisionState: (event) => this.threadCommands.handlePlanDecisionState(event),
      handlePlanDecisionSnapshot: (sessionKeyValue, gatewayEpoch, decisions) => this.threadCommands.handlePlanDecisionSnapshot(sessionKeyValue, gatewayEpoch, decisions),
      handleSharedGoalState: (sessionKeyValue, goal) => this.threadCommands.handleSharedGoalState(sessionKeyValue, goal),
      handleSharedCommandState: (event) => this.threadCommands.handleSharedCommandState(event),
      appendSystem: (conversationId, text) => this.appendSystem(conversationId, text),
      sendRendered: (conversationId, rendered, options) => this.sendRendered(conversationId, rendered, options),
